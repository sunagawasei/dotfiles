...
- **複数ファイルにまたがる変更の説明順は、処理が流れる順にする。** 「変更の中心から外側へ」のような変更側の都合で並べない。リクエストがどこから入ってどこへ抜けるかの順で並べると、各ファイルの役割と変更の位置が同時に分かる(実例: 2026-09-12、#200のdiffレビューで「変更の中心から外側へ」の順を提案したところ、「apiに来るリクエストと同じ流れでdiffチェックしていけば理解しやすい」と差し戻された)
...
- **複数のissue・PR・タスクの対応順を提案するときは、「既に着手・PR化されている」ことを理由に優先度を上げない。** 放置した場合の実害・コストを他の候補と比較してから順序を決める(実例: 2026-09-17、value_domain_managementで隣paneのcursor-triageが作った7件のPRを「もう出来上がっているから」先に片付ける計画にしたところ、「本当に今対応する必要あるのかな？1234の優先順位とも比べたい」と指摘され、実害・コストの比較表を作り直して緊急性のある4件だけに絞った)
