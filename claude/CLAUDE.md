...
- **AskUserQuestionの前に選択肢を本文で明示する**: ダイアログを出す前に、どういう選択肢があり各々何を意味する（選ぶと何が起きる）かを本文テキストに書く。ダイアログの中だけに選択肢情報を持たせない（2026-07-13ユーザー指示）
- **選択肢・分岐点の提示では、初出の内部名(ファイル名・コンポーネント名等)に一言の役割説明を添える**: ユーザーが「それは何?」と聞き返さずに各選択肢の意味を判断できる状態にしてから選ばせる(2026-07-13ユーザー指示)
- **長い成果物と同一ターンでAskUserQuestionを出さない**: レポート・棚卸し・調査結果など読むのに時間がかかる出力をしたターンでは、ダイアログが即座に被さって読む時間を奪うため、選択肢は本文のテキストで示してターンを終える。ユーザーが読了後に自由回答するか「askuserquestion」で選択肢化を求めるのを待つ。AskUserQuestionを即出ししてよいのは直前の出力が短く読む負担がない場合のみ
...
- **実質的な技術回答はcodexに裏どりさせてから出す**（2026-07-13指示・全プロジェクト適用）: 技術主張・設計判断・調査結果を含む回答は、codex-research（事実・実装確認）またはcodex（査読）の検証を通してから出す。裏どり前に見解を出す場合は「未裏どり」と明記する。挨拶・進行連絡など事実主張を含まないターンは対象外
