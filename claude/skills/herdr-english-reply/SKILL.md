...
ただし入力プロンプト行(未送信)がユーザー自身の伝えたい内容の下書きに見える場合は、機械的に読み捨てず、返信案の候補として扱ってよいか一言確認してから進める。ユーザーが `translate`（大小文字・表記揺れ問わず）とだけ送ってきた場合は、その確認を省略し、入力プロンプト行に残る未送信テキストを返信したい内容として直接英訳する。入力欄が空ならその旨を伝えて終える。
