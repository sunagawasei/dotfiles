...
## claude を spawn して委譲・検証するときの落とし穴

長文プロンプトのペースト扱い、`❯`プロンプトの検知、agent_statusポーリングでの完了検知、権限プロンプトの扱いなど、`claude`をspawnして委譲・設定検証する際に踏みやすい非自明な事象は [`references/troubleshooting.md`](references/troubleshooting.md) にまとめてある。spawnした先が動かない・タイムアウトする・submitされない、といった事象に当たったら読む。
