...
実際の調査は各 CLI（codex/cursor）が CWD で自律実行する。
...
> 各CLIの canonical form・フラグ・timeout は各エージェント定義（`agents/{codex,cursor}.md`）が管理する。オーケストレータは subagent を起動するだけで CLI コマンドを直接組み立てない。
