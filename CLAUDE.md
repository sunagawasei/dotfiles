...
- **Cursor CLI**: `cursor/` - AI統合ターミナルエディタ、Vimモード対応

### カラーシステム

- **統一テーマ**: カスタムダークテーマ
- **ドキュメント**: `COLOR-SYSTEM.md`
...
- **Cursor CLI**: `cursor/cli-config.json` - メイン設定ファイル
...
- **設定ディレクトリ**: `cursor/` - Cursor CLI設定

### 設定ファイル

- **メイン設定**: `cursor/cli-config.json` - エディタ、モデル、セキュリティ設定
- **ドキュメント**:
  - `cursor/README.md` - 概要と使用方法
  - `cursor/KEYMAPS.md` - キーバインドリファレンス
  - `cursor/SETTINGS.md` - 詳細設定ガイド

### 主な機能

- **Vimモード**: 有効（標準的なVimキーバインド）
- **AIモデル**: GPT-5.2 Codex Extra High
- **セキュリティ**: allowlist方式（許可リストベース）
- **カラーテーマ**: WezTermのANSI色設定に準拠
