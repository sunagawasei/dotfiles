...
- **Nixの`wrapNeovim`は起動時に`--cmd "lua vim.g.loaded_..._provider=0;..."`を`$@`の前に固定注入する**。`vim.v.argv`をargv[2]から素朴に走査してフラグ判定するロジックは、この`--cmd`を常にユーザーのフラグと誤検知する。`--cmd`はNixラッパーの固定payload文字列と厳密一致する場合だけpayloadとともにスキップし、不一致なら出現順に依存せずその場でnestする。未知の`--cmd`をhostへfail-openさせない設計である（`nvim/lua/util/flatten_classify.lua`参照）
